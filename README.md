...
| `Segments.Workers` | `0` (`DefaultSegmentWorkers`) | ASR 分片导出和编码并发数；`0` 表示自动 |
...
| `Libav.CodecThreads` | `0` (`DefaultLibavCodecThreads`) | 单个 libav pipeline 的 decoder/encoder 线程数；`0` 表示使用 libav 默认策略 |
...
`Libav.CodecThreads` 会传给每个内置 libav pipeline 的 decoder 和 encoder。默认 `0` 保持 FFmpeg/libav 的默认线程策略；显式设置大于 `0` 的值时，需要同时考虑 `FixedTrim.Workers` 和 `Segments.Workers`，避免 Go 层多 worker 与 C 层 codec 线程叠加后过量并发。
...
| `--segment-workers` | `0` | ASR 分片导出和编码并发数；`0` 表示自动 |
| `--libav-codec-threads` | `0` | 单个 libav pipeline 的 decoder/encoder 线程数；`0` 表示使用 libav 默认策略 |
...
```json
{
  ...
  "info": {
    ...
    "segment_skipped": 0,
    "output_files": [
      "/tmp/segments/sa_7f3a2c1d9b0e4a6c_part001.ogg",
      "/tmp/segments/sa_7f3a2c1d9b0e4a6c_part002.ogg"
    ]
  },
  "segments": [],
  "steps": {
...
默认 FFmpeg 源码为官方稳定版 `8.1.2`：

```text
https://www.ffmpeg.org/releases/ffmpeg-8.1.2.tar.gz
```

脚本会下载对应的 `.asc` 签名和 FFmpeg release PGP key，先校验公钥指纹，再验证源码包签名。Opus 源码包仍通过 SHA256 校验。
