...
| `Segments.SampleRate` | `16000` (`DefaultSampleRate`) | 输出采样率 |
...
ASR 分片输出可以按端点要求调整容器、编码器、码率和采样率。声道固定为合并后的单声道：
...
| `wav` | `pcm_s16le` | `.wav` | 无压缩 PCM，忽略码率 |
| `flac` | `flac` | `.flac` | 无损压缩，忽略码率 |
| `aac` / `adts` | `aac` | `.aac` | ADTS AAC |
| `m4a` / `mp4` | `aac` | `.m4a` | MP4/M4A AAC |

如果只设置 `OutputFormat`，库会按上表选择默认编码器。`OutputBitrate` 只对 `libopus`、`opus`、`aac` 生效；`pcm_s16le` 和 `flac` 会自动忽略码率。
...
| `--output-codec` | `libopus` | ASR 分片输出编码器，例如 `libopus`、`pcm_s16le`、`flac`、`aac` |
