...
The Go `libav` backend links against C functions named `sa_*`.
...
- `sa_volume_detect`: decode audio and run the FFmpeg `volumedetect` filter.
- `sa_silence_detect`: decode audio and run the FFmpeg `silencedetect` filter with the supplied threshold and duration.
...
- `sa_render_intervals_wav`: build an `atrim + asetpts + concat` filtergraph from ordered intervals and render mono WAV.
...
The C layer owns FFmpeg object lifetimes and returns heap-allocated interval arrays to Go. Go releases those arrays through `sa_free`.

The C layer should avoid global mutable FFmpeg log capture for concurrent calls unless protected or made thread-local. Fixed-slice trimming depends on backend calls being safe under concurrent Go workers.
